    ...
    class Formula
    {
        ...
        public $aux;                    // hold temp values
          ...
          public function section_1_D($table, $row_style, $style, $size)
          {
               ...
               $this->new_table_row($table, 'value', $style, 4, 'Consulta de profissional de nivel superior na atenção básica (odontológica)', $this->total_c_1*2);
          ...
          {
               $this->new_table_row($table, 'value', $style, 4, 'Consulta médica em atenção básica', $this->total_c_2*2);
               $this->new_table_row($table, 'value', $style, 4, 'Consulta de profissional de nivel superior na atenção básica', $this->total_c_2*1);
               $this->new_table_row($table, 'value', $style, 4, 'Consulta médica em atenção especializada (consulta de especialidades)', 'De acordo com diagnóstico');
               $this->new_table_row($table, 'value', $style, 4, 'Atividade educativa/Orientação em grupo na atenção básica', '1 a.e/população coberta/ano');
               $this->new_table_row($table, 'value', $style, 4, 'Vacinação básica', 'De acordo com diagnóstico');
               $this->new_table_row($table, 'value', $style, 4, 'Exames', 'De acordo com diagnóstico');
               $this->new_table_row($table, 'value', $style, 4, 'Consulta/Atendimento de reabilitação', 'De acordo com diagnóstico');
               $this->new_table_row($table, 'value', $style, 4, 'Consulta para acompanhamento de crescimento e desenvolvimento', 'De acordo com diagnóstico');
          }
